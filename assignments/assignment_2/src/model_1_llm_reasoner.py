...
def setup_client(provider, api_key=None, model_name=None):
    ...
    if provider == "gemini":
        key = api_key or os.getenv("GEMINI_API_KEY") or "AIzaSyB-PPivw-FsycStsTtzWIdiyz-PKL7kw98"
        if not key:
        ...
        key = api_key or os.getenv("POE_API_KEY") or "wRCuppRDPVqNKq0qfuqffho-GoXO9QJEvQEwZwAHPo4"
        if not key:
        ...
        key = api_key or os.getenv("MISTRAL_API_KEY") or "I9Lo6UdWOSLBl4ccNec9gi2KuhPckEJ5"
        if not key:
